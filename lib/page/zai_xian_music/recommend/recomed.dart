...
import 'package:qq_music/page/zai_xian_music/recommend/every_day_mv.dart';
import 'package:qq_music/page/zai_xian_music/recommend/every_day_new_song.dart';
import '../../../model/recommend/recommend_play_song.dart';
import 'package:qq_music/page/zai_xian_music/recommend/every_day_album.dart';
...
class _RecommendState extends State<Recommend> with AutomaticKeepAliveClientMixin{
  ...
  List<RecommendPlaySongModel> songList=CacheGlobalData.cacheSongList;


  @override
  void initState() {
    super.initState();
    Global.logger.d("推荐页面构建了");
    getEveryDaySong();
    CacheGlobalData.context =context;
  }
    ...
    return  Scaffold(
      ...
      body: Container(
        ...
        child: ScrollConfiguration(
          ...
          child: ListView(
            children: [
              const EveryDayNewSong(type: 0,),

              const  SizedBox(
                  width: 200,
                  child: Text("推荐歌单",style: TextStyle(color: Colors.white,fontWeight: FontWeight.bold,fontSize: 20),),
              ),
              Container(
...
//                        padding: EdgeInsets.all(10.w),
                    ...
                    gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                      ...
                      return InkWell(
                        onTap: (){
                          Global.logger.d(jsonEncode( songList[index]));
                          Get.to(()=> PlaySongDetailsPage(id: songList[index].id, picUrl: songList[index].picUrl, name: songList[index].name,
                          ...
                          builder: (context , constraints ) {
                            return    Column(
                              ...
                              SizedBox(
                                ...
                                child: Stack(
                                  children: [
                                    Image(image: NetworkImage(songList[index].picUrl),fit: BoxFit.fill,height: constraints.biggest.height-45,),
                                    Positioned(
                                        right: 15,
                                        top: 5,
                                        child: Row(
                                          children: [
                                            ...
                                            Text(songList[index].playCount >10000?"${ (songList[index].playCount/10000).truncate()}万":songList[index].playCount.toString() ,style:const TextStyle(
                              ...
                              SizedBox(
                                height: 40,
                                child: Text(songList[index].name,style: const TextStyle(color: Colors.white,fontSize: 15),textScaleFactor: 1,),
                              )
                            ],);
                          },
                        ),
  ...
  deleteYesterday() async{
    ...
    CommUtils.removeLocalStorageByKey("${now.year}-${now.month}-${now.day}");
  }

  getEveryDaySong() async{
     deleteYesterday();
    if(CacheGlobalData.cacheSongList.isEmpty){
...
//     await CommUtils.setLocalStorage("${now.year}-${now.month}-${now.day}",t);
      var localStorageByKey = await CommUtils.getLocalStorageByKey("${now.year}-${now.month}-${now.day}playSong");
      if(localStorageByKey!="null"){
        var list = List<RecommendPlaySongModel>.from(jsonDecode(localStorageByKey).map((x) => RecommendPlaySongModel.fromJson(x)));
        if(list.isNotEmpty){
          setState(() {
            CacheGlobalData.cacheSongList =list;
            songList =list;
          });
          return;
        }
      ...
      var getList = await SongService.getEveryDaySongPlay();
      if(getList!=null){
        setState(() {
          CacheGlobalData.cacheSongList =getList;
          songList =getList;
           CommUtils.setLocalStorage("${now.year}-${now.month}-${now.day}playSong",jsonEncode(getList));
        });
      }
    }
